...
export class CandidateEditComponent implements OnInit {
  ...
  constructor(
    ...
    this.fileurl = environment.imgUrl;
  ...
  public async get_candidate() {
    ...
    try {
      ...
      if (res?.status == true) {
        ...
        const profession = this.result.professions.filter((x: any) => x.id === candidate?.profession_id);
        const education = this.result.educations.filter((x: any) => candidate?.education.includes(String(x.id)));
        const skills = this.result.skills.filter((x: any) => candidate?.skills.includes(String(x.id)));
        const experience = this.options.experience.filter((x: any) => x.value === String(candidate?.experience));
        const job_role_id = this.result.job_roles.filter((x: any) => x.id === candidate?.job_role_id);
        const gender = this.options.gender.filter((x: any) => x.value === candidate?.gender);
        const languages = this.options.languages.filter((x: any) => candidate?.languages.includes(String(x.value)));
        const marital_status = this.options.marital_status.filter((x: any) => x.value === candidate?.marital_status);
        this.resume.preview = candidate?.resume ? this.fileurl + candidate.resume : 'assets/images/pdf.png';
  ...
  public async dosubmit() {
    ...
    this.Form.get('languages')?.value.length ? this.makearr(this.Form.get('languages').value).forEach((item: any, i: any) => { data[`languages[${i}]`] = item }) : data['languages[]'] = '';
    this.err = {};
    try {
